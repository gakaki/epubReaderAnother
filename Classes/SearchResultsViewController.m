...
- (void) searchString:(NSString *)query inChapterAtIndex:(int)index{
    ...
    Chapter* chapter = (epubViewController.loadedEpub.spineArray)[index];
...
//    NSLog(@"w:%f h:%f", webView.bounds.size.width, webView.bounds.size.height);
	...
	NSString *setTextSizeRule = [NSString stringWithFormat:@"addCSSRule('body', '-webkit-text-size-adjust: %d%%;')",[(epubViewController.loadedEpub.spineArray)[currentChapterIndex] fontPercentSize]];
